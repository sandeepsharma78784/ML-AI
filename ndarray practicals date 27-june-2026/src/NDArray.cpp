...
InfixExpression<NDArray *,NDArrayTransposedView *> NDArray::operator*(NDArrayTransposedView &right)
{
	return InfixExpression<NDArray *,NDArrayTransposedView *>(this,'*',&right);
}
...
InfixExpression<NDArray *,double> NDArray::operator^(double value)
{
	return InfixExpression<NDArray *,double>(this,'^',value);
}
...
InfixExpression<double,NDArray *> NDArray::operator-()
{
	return InfixExpression<double,NDArray *>(-1.0,'*',this);
}
...
NDArray NDArray::operator=(InfixExpression<double,NDArray *> expression)
{
	if(expression.oper=='-') this->subtract(expression);
	if(expression.oper=='*') this->multiply(expression);
	return *this;
}
NDArray NDArray::operator=(InfixExpression<NDArray *,double> expression)
{
	if(expression.oper=='^') this->pow(expression);
	return *this;
}
...
NDArray NDArray::operator=(InfixExpression<NDArray *,NDArrayTransposedView *> expression)
{
	if(expression.oper=='*') this->multiply(expression);
	return *this;
}
...
NDArray NDArray::operator=(InfixExpression<NDArrayView *,NDArray *> expression)
{
	if(expression.oper=='.') this->pointwise_product(expression);
	return *this;
}
...
void NDArray::pointwise_product(InfixExpression<NDArrayView *,NDArray *> &expression)
{
size_t sz=this->dimensions.size();
int size_of_one_array=expression.left->to[sz-1]-expression.left->from[sz-1]+1;
int i,k;
i=0;
k=0;
uint32_t idx=0;
...
{
e=expression.left->ndArray->collection[expression.left->base_indices[i]+k]*expression.right->collection[idx++];
k++;
...
void NDArray::subtract(InfixExpression<double,NDArray *> &expression)
{
int i=0;
for(auto &t:this->collection) 
{
	t=expression.left-expression.right->collection[i];
	++i;
}
}
void NDArray::pow(InfixExpression<NDArray *,double>  &expression)
{
uint32_t idx=0;
for(auto &t:this->collection)
{
t=::pow(expression.left->collection[idx++],expression.right);
}
}

void NDArray::multiply(InfixExpression<double,NDArray *>  &expression)
{
uint32_t idx=0;
for(auto &t:this->collection)
{
t=expression.left*expression.right->collection[idx++];
}
}
...
void NDArray::multiply(InfixExpression<NDArray *,NDArrayTransposedView *> &expression)
{
auto left=expression.left;
auto left_array=left->collection.data(); 
auto right=expression.right;
auto right_array=right->ndArray->collection.data();
auto left_dimensions=left->dimensions;
uint32_t i;
uint32_t left_matrix_rows;
uint32_t left_matrix_columns;
...
uint32_t right_matrix_len;
int number_of_right_matrices;
auto left_matrix_ndarray_dimensions=left->dimensions;
...
{
// over here, just copy paste, whatever is below
...
{
double s=0;
for(auto x:this->collection) s+=x;
return s;
}
