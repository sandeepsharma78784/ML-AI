/*
used shared pointer to avoid deep copy of collection
*/
...
private:
  ...
  public:
    NDArray(uint32_t rows, uint32_t cols)
        : dimensions{rows, cols},collection(make_shared<vector<int>>(rows * cols)),isTransposed(false) {}
...
int main() 
{
    ...
    NDArray t = m.transpose();  // no new copy of collection is created because of shared pointer.
    cout << "Transposed:\n";
    t.print();
	
    // change the value in t
